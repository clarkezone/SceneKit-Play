...
final class SceneController : UIViewController {
    ...
    func buildScene() {
        ...
        //Image
        //let jcTexture = SKTexture(imageNamed: "test.jpg")
        //var cool = SKTexture.init(cgImage: image as! CGImage)
        //let pic = SKSpriteNode(fileNamed: "test.jpg")
        //pic?.position = CGPoint(x:200, y:200)
        //pic?.physicsBody = SKPhysicsBody(rectangleOf: (pic!.texture?.size())!)
        //myScene.addChild(pic!)
        // ground::
        var splinePoints = [CGPoint(x:0, y: 0), CGPoint(x:250, y:100),
                            CGPoint(x:400, y:50),
                            CGPoint(x:500, y:100),
                            CGPoint(x:525, y:0),
                            ]
